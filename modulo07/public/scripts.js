...
const PhotosUpload = {
    ...
    handleFileInput(event) {
		...
		PhotosUpload.updateGetAllFiles();
    },
    hasLimit(event) {
    ...
    getCreateButtonRemove() {
        ...
        button.onclick = () => PhotosUpload.removePhoto(event);
    ...
    removePhoto(event) {
        ...
        const newFiles = Array.from(PhotosUpload.previewDiv.children).filter((file) => {
            if(file.classList.contains('photos') && !file.getAttribute('id')) return true;
        });
